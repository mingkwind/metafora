...
func PutMetadata(metadata *Metadata) error {
	...
	_, err = elasticClient.Index().
		...
		BodyJson(metadata).
		Do(ctx)
	// 如果是es7的restful api,则相当于put /metadata/_doc/name_version
	if err != nil {
		return err
	}
	return nil
...
func DelMetadata(name string, version int) error {
	...
	_, err := elasticClient.Delete().
		...
		Id(id).
		Do(ctx)
	// 相当于es的delete /metadata/_doc/name_version
	if err != nil {
...
func GetMetadata(name string, versionId int) (Metadata, error) {
	...
	if versionId == 0 {
		...
		get, err := elasticClient.Get().
			...
			Id(id).
			Do(ctx)
		// 相当于es的get /metadata/_doc/name_version
		if err != nil {
			return Metadata{}, err
		}
		if get.Found {
...
func GetLatestVersion(name string) (Metadata, error) {
	...
	// 相当于
	/*
			curl -X GET "http://localhost:9200/metadata/_search?pretty=true" -H 'Content-Type: application/json' -d '{
		  "query": {
		    // 在此处插入您的查询条件
		  },
		  "sort": [
